import { MoveUpRight, Plus, X, ChevronDown } from "lucide-react";
...
// Items that get a chevron in mobile (same as plus items on desktop)
const mobileExpandable = ["Services", "Industries", "International", "About", "Blog & Resources"];

const mobileItems = [
  { label: "Services", expandable: true },
  { label: "Industries", expandable: true },
  { label: "International", expandable: true },
  { label: "About", expandable: true },
  { label: "Work" },
  { label: "Careers" },
  { label: "Blog & Resources", expandable: true },
  { label: "Webinar" },
];
    ...
    <>
      <nav className="flex items-center justify-between px-6 py-6">
        ...
        <div className="flex items-center text-white cursor-pointer">
          <svg
            className="h-[18px] mr-6 w-auto md:h-[20px] fill-current"
        ...
        <ul className="hidden md:flex items-center">
          {navItems.map((item) => (
            <li
              ...
              className="relative"
            >
              <button className="relative z-10 flex items-center gap-1 px-4 py-[7px] rounded-full text-[15px] font-medium text-white transition-colors duration-300 hover:text-black">
                {hovered === item.label && (
                  <motion.span
                ...
                {item.plus && (
                  <Plus size={13} strokeWidth={3.2} className="relative z-20" />
                )}
                {item.badge && (
                  <span className="absolute -top-1 -right-1 bg-[#b4ff39] text-black text-[10px] font-bold px-1.5 py-0.5 rounded-full leading-tight z-20">
                    {item.badge}
                  </span>
                )}
              </button>
        ...
        {/* Desktop CTA */}
        <button className="hidden md:flex group relative items-center gap-2 overflow-hidden bg-white text-black px-4 py-2 rounded-full font-bold hover:rounded-xl transition-[border-radius] duration-300">
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            <motion.div
              key="drawer"
              className="fixed inset-0 z-50 flex flex-col bg-[#1a1a1a]/35 backdrop-blur-md"
          ...
          >
            ...
            <path d="M165.127 10.1622C166.714 10.1622 168 8.87583 168 7.28913C168 5.70242 166.714 4.41602 165.127 4.41602C163.54 4.41602 162.254 5.70242 162.254 7.28913C162.254 8.87583 163.54 10.1622 165.127 10.1622ZM165.127 5.22763C166.264 5.22763 167.189 6.15219 167.189 7.28913C167.189 8.42606 166.264 9.35062 165.127 9.35062C163.99 9.35062 163.066 8.42606 163.066 7.28913C163.066 6.15219 163.99 5.22763 165.127 5.22763Z" />
              ...
              <div className="flex-1 overflow-y-auto">
                {mobileItems.map((item, i) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between px-5 py-[18px] border-b border-white/10 cursor-pointer group"
                  >
              ...
              <div className="px-5 py-5 border-t border-white/10">
                <button className="group w-full flex items-center justify-center gap-2 bg-white text-black px-5 py-4 rounded-full font-bold text-[15px] hover:rounded-xl transition-[border-radius] duration-300 overflow-hidden relative">
                  <span className="relative overflow-hidden h-[1.2em] flex items-center">
                    <span className="block whitespace-nowrap translate-y-0 group-hover:-translate-y-[110%] transition-transform duration-[400ms] ease-[cubic-bezier(0.22,1,0.36,1)]">
                      Get In Touch
                    </span>
